...
> **為何只在 HUD 預覽、放開才鍵入**：Whisper 串流時會修正前面猜錯的字（非單調）。
> 若邊講邊把字打進你的編輯器，遇到修正就得倒退刪字——在任意 app 裡可能誤刪別的內容。
> 所以串流期間只在 HUD 泡泡顯示，**放開才一次鍵入最終文字**（此時大部分音訊已辨識完，
> 定稿幾乎瞬間，鍵入零延遲感），編輯器永遠不會被倒退刪字。
...
2. **刪掉**範本的 `ContentView.swift` 和預設 `VibeVoiceApp.swift`，把本資料夾 `VibeVoice/` 裡的 8 個 `.swift` 拖進去。
...
`transcribe(audioArray:decodeOptions:) -> [TranscriptionResult]`、
`AudioProcessor()` / `startRecordingLive(inputDeviceID:callback:)` / `stopRecording()` / `audioSamples: ContiguousArray<Float>`、
